					...
					<div className="card mb-3 shadow h-100">
						...
						<div className="card-body">
							...
							{!KindlerList?.[0]?.friend ? (
								...
								<div>
									<h6 className="card-text">How to contact {KindlerList?.[0]?.username}: {KindlerList?.[0]?.email}</h6>
                  <div style={{ padding: 5 }}>
									<h5 className="card-subtitle mb-2 text-muted"> {KindlerList?.[0]?.username} added to friends list! </h5>
                  </div>
								</div>
							)}
						</div>
					...
					<div className="row">
						{KindlerList.slice(1).map((KindlerUser, index) => {
							console.log(index);
							return (
                      ...
                      {/* <div className="align-middle"> */}
											...
											{!KindlerUser?.friend ? (
												<button
													...
													className="btn btn-warning"
													onClick={() => {
														onClick(KindlerUser?.id, index + 1);
													}}
												>
													{" "}
													Add to Friend's List{" "}
                        ...
                        <div>
                          <h6 className="card-text">How to contact {KindlerUser?.username}: {KindlerUser?.email}</h6>
                          <div style={{ padding: 5 }}>
